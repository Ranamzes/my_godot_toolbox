...
## Project setting key for the UTC Unix timestamp of the jam end date.
const SETTING_UTC_UNIX := SETTINGS_PREFIX + "end_date_utc_unix"
...
func _load_settings() -> void:
	...
	if ProjectSettings.has_setting(SETTING_UTC_UNIX):
		# New format: UTC Unix timestamp (timezone-safe).
		jam_date_unix = ProjectSettings.get_setting(SETTING_UTC_UNIX)
		var offset := _get_utc_offset()
		var local_dict := Time.get_datetime_dict_from_unix_time(jam_date_unix + offset)
		year = local_dict.year
		month = local_dict.month
		day = local_dict.day
		hour = local_dict.hour
		minute = local_dict.minute
	else:
		# Legacy format: individual date components (not timezone-safe).
...
func initialize_countdown() -> void:
	if jam_date_unix == 0:
		# Compute from local components (default/legacy path).
		jam_date_unix = Time.get_unix_time_from_datetime_dict(jam_end_date) - _get_utc_offset()
	var current_utc_unix := int(Time.get_unix_time_from_system())

	time_left_unix = jam_date_unix - current_utc_unix
	if time_left_unix <= 0:
...
## Returns the local UTC offset in seconds (local_time = utc_time + offset).
## For example, UTC+6 returns 21600.
func _get_utc_offset() -> int:
	var now_utc_unix := int(Time.get_unix_time_from_system())
	var now_local_dict := Time.get_datetime_dict_from_system(false)
	var now_local_as_utc_unix := Time.get_unix_time_from_datetime_dict(now_local_dict)
	return now_local_as_utc_unix - now_utc_unix
